...
fun HomeScreen(
    ...
    Scaffold(
        ...
        if (isDownloading){
            DownloadingScreen(progress,isDownloading) {
                viewModel.getAllAyahtsFromApi()
            }
        } else {
            CategoryButtons(navController,paddingValue)
        }
...
@Composable
fun CategoryButtons(navController: NavController, paddingValue: PaddingValues) {
    Column(
        modifier = Modifier
            .fillMaxSize()
